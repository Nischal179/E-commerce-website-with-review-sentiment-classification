							...
							<?php
								$sql = "SELECT * FROM PRODUCT_CATEGORY";
									...
									while ($row = oci_fetch_assoc($result)){
										...
										$catName = $row['NAME'];
								?>
								<a href="browse_by_categories.php?cid=<?php echo $catid; ?>" class="list-group-item <?php if ($category_id==$catid) { echo "active"; } ?>">
									<?php echo $row['NAME']; ?>
								</a>
								<?php
							}
				...
				<div id="main" class="col-md-9">			
					...
					<div id="store">
						...
						<div class="row">
							...
							<?php
								$numberOfProducts = 0;
								$sql3 = "SELECT * FROM SHOP WHERE IS_ACTIVE='1'";
							...
							<?php
								...
								$sql = "SELECT * FROM product WHERE PRODUCT_SHOP_ID=$SHOP_ID AND PRODUCT_CATEGORY_ID=$category_id";
								$result = oci_parse($conn,$sql);
								oci_execute($result);								
								while ($row = oci_fetch_assoc($result)){
